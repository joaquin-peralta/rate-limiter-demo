...
**This app is using Rails 8.0.2 and Ruby 3.4.4**. The screenshot shows
...
```sh
git clone https://github.com/nickjj/docker-rails-example rate_limiter
cd rate_limiter
...
The app is named `rate_limiter` right now but chances are your app will be a different
name. Since the app is already created we'll need to do a find / replace on a
few variants of the string "rate_limiter" and update a few Docker related resources.
